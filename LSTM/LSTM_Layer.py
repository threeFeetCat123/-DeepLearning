...
class LstmLayer(object):
    def __init__(self, input_width, state_width, output_width, learning_rate):
        ...
        if(state_width != output_width):
            raise ValueError("state_width must be equal to output_width", state_width, output_width)
...
def test_lstmlayer():
    # 初始化 LSTM 层
    lstm_layer = LstmLayer(1, 5, 5, 0.1)  # 输入宽度1，状态宽度1，学习率0.01
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_sequences):
            ...
            lstm_layer.backward(input_x, expected_y)

            # 计算损失
            total_loss += np.mean(np.abs(y_train[i][-1] - lstm_layer.h_list[-1].T))
